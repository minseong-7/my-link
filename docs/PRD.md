...
2. **프로필 관리**
   - **DisplayName (URL ID)**: 고유한 사용자 식별자, URL 경로로 사용 (예: `myLink.com/displayName`)
   - 초기값: Google 이메일(@ 앞부분)을 기본값으로 사용 (중복 시 숫자 등 추가 처리).
   - **Username (사용자 이름)**: 프로필 상단에 표시되는 실제 이름.
   - 인라인 편집: 별도의 수정 폼/모달 없이 텍스트를 클릭하여 즉시 수정.
   - 수정 가능 항목: `DisplayName(ID)`, `Username(이름)`, 한 줄 소개(`Bio`).
3. **링크 블록 관리**
   - 링크 추가/수정/삭제.
...
- **로직**: Firebase Auth.
- **초기값**:
  - `DisplayName (URL)`: Google 이메일 아이디(`user` from `user@gmail.com`)로 자동 설정.
  - `Username (이름)`: Google 계정의 이름(Name)으로 설정.
  - `ProfileImage`: Google 계정 사진 사용.

## 3.2. 마이페이지 (Admin) - 인라인 에디팅 중심
- **프로필 영역**:
  - 프로필 사진(Google URL), Username(이름), DisplayName(@id), 소개글 표시.
  - 텍스트 클릭시 `contenteditable` 혹은 `input` 으로 전환되어 즉시 수정 (Inline Edit).
  - 중복 검사: DisplayName 수정 시 유일성(Uniqueness) 검사 필요.
- **링크 목록 영역**:
  - **"새 링크 추가" 버튼**: 클릭 시 리스트 하단에 빈 입력 필드 생성.
  - **리스트 아이템**:
    - 아이콘: URL 입력시 `https://www.google.com/s2/favicons?domain=url` 형식을 이용하여 자동 렌더링.
...
```
*Note: 파비콘은 DB에 저장하지 않고 클라이언트에서 URL을 기반으로 동적 생성.*
